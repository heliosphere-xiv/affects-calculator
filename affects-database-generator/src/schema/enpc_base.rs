...
impl MetadataExtractor for ENpcBase {
    ...
    fn populate_row(row: ironworks::excel::Row) -> Result<Self, Self::Error> {
        let item = crate::populate!(
            ...
            [model_body, 76, into_u32],
            [model_hands, 79, into_u32],
            [model_legs, 82, into_u32],
            [model_feet, 85, into_u32],
            [model_ears, 88, into_u32],
            [model_neck, 91, into_u32],
            [model_wrists, 94, into_u32],
            [model_left_ring, 97, into_u32],
            [model_right_ring, 100, into_u32],
            row_id: row.row_id(),
        );
