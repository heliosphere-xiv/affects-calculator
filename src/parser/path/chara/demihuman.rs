...
use crate::parser::{
    EquipSlot, GamePath, IResult, check_repeat_id, equip_slot, n_digit_id, path_id,
};
...
fn mdl_path(primary_id: u16, secondary_id: u16) -> impl Fn(&str) -> IResult<&str, DemihumanPath> {
    move |input: &str| {
        map_res(
            ...
            |((repeat_primary_id, repeat_secondary_id), slot)| {
                check_repeat_id(primary_id, repeat_primary_id)?;
                check_repeat_id(secondary_id, repeat_secondary_id)?;
                anyhow::Ok(DemihumanPath::Mdl {
                    primary_id,
                    secondary_id,
                    slot,
                })
...
fn mtrl_path(primary_id: u16, secondary_id: u16) -> impl Fn(&str) -> IResult<&str, DemihumanPath> {
    move |input: &str| {
        map_res(
            ...
            |(variant_id, ((repeat_primary_id, repeat_secondary_id), slot))| {
                check_repeat_id(primary_id, repeat_primary_id)?;
                check_repeat_id(secondary_id, repeat_secondary_id)?;
                anyhow::Ok(DemihumanPath::Mtrl {
                    primary_id,
                    secondary_id,
                    variant_id,
                    slot,
...
fn chara_demihuman_path_skeleton(input: &str) -> IResult<&str, GamePath> {
    map_res(
        ...
        |(primary_id, secondary_id, (repeat_primary_id, repeat_secondary_id))| {
            check_repeat_id(primary_id, repeat_primary_id)?;
            check_repeat_id(secondary_id, repeat_secondary_id)?;
            anyhow::Ok(GamePath::Demihuman(DemihumanPath::Skeleton {
                primary_id,
                secondary_id,
            }))
        },
